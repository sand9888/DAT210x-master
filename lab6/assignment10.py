...
zero1 = []
wavef = []
#rate, test = wavfile.read('https://raw.githubusercontent.com/Jakobovski/free-spoken-digit-dataset/master/recordings/0_jackson_0.wav')
for i in range(0,49,1):
	zero1.append(wavfile.read('C:/Users/sand9888/Downloads/free-spoken-digit-dataset-master/0_jackson_' + str(i) + '.wav')[1])
	#print(zero1)
...
zero = pd.DataFrame(zero1, dtype=np.int16)
#wave_df = pd.DataFrame(wavef, dtype=np.int16)
...
#
zero = zero.dropna(axis=1).values
...
wavfile.write('Original Test Clip.wav', 8000, test)
...
y_train=[]
for row in train:
	X_train.append(row[:int(Provided_Portion * n_audio_samples)])
	y_train.append(row[int(Provided_Portion * n_audio_samples):])
...
#
y_test_prediction = model.score(X_test, y_test)
