...
def quantile_number(quant_number=5, min_est_level = 10000):
    ...
    df_sum_month['Median'] = df_final.groupby(['Month'])['Average-error'].transform('median')
    df_sum_month['Mean'] = df_final.groupby(['Month'])['Average-error'].transform('mean')
    df_sum_month['20_percentile'] = df_final.groupby(['Month'])['Average-error'].transform(lambda x: x.quantile([0.2]))
    df_sum_month['80_percentile'] = df_final.groupby(['Month'])['Average-error'].transform(lambda x: x.quantile([0.8]))
    
    '''for month in df_sum_month['Month']:
        print(month)'''
     #   df_sum_month.loc[month, 'Estimation_%_median'] = df_final.Average%[df_final == month].median
    # df_sum_month.loc[df_final.Month == iii, 'Estimation_%']
