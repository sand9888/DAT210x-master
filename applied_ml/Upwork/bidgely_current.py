...
def quantile_number(quant_number=5):
    ...
    for id in list(df_final['QuantileId'].unique()):
        ...
        # variant3
        result = pd.merge(df_final[(df_final['QuantileId'] == id)], df_non_raw, on=['UUID', 'Month'], how='inner')
        result['QuantileId'] = id
        print(result)
    ...
    for ii in list_quant:
        df_final['Average%'] = df_final['Average'] / df_non_raw.loc[
            (df_non_raw['QuantileId'] == ii) & (df_final['QuantileId'] == ii), 'Consumption'].mean()
        df_final['Median%'] = df_final['Average'] / df_non_raw.loc[
            (df_non_raw['QuantileId'] == ii) & (df_final['QuantileId'] == ii), 'Consumption'].median()
