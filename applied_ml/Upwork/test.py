...
def quantile_number(quant_num = 5):
	...
	for i, date_month  in enumerate(df_final['Month']):
		for j, quant_id in enumerate(df_non_raw['quantileId']):
			# print(i, date_month, j, quant_id)
			if (i > 1) and (j > 1) and (df_final.ix[i, 'Month'] == df_non_raw.ix[j, 'Month']) and (df_final.ix[i, 'UUID'] == df_non_raw.ix[j, 'UUID']):
