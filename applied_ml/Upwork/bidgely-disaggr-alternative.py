...
for i in list(df.NhoodId.unique()):
    ...
    df_2quant = df_id[(df_id.Consumption > np.percentile(df.Consumption, 20)) & (
    df_id.Consumption <= np.percentile(df.Consumption, 40))]
    df_2quant.loc[:, 'quantileId'] = 2

    df_3quant = df_id[(df_id.Consumption > np.percentile(df.Consumption, 40)) & (
    df_id.Consumption <= np.percentile(df.Consumption, 60))]
    df_3quant.loc[:, 'quantileId'] = 3

    df_4quant = df_id[(df_id.Consumption > np.percentile(df.Consumption, 60)) & (
    df_id.Consumption <= np.percentile(df.Consumption, 80))]
...
# computing descriptive statistic
    ...
    for x in range(1, 6):
        quant = df_final[df_final.quantileId == x]
