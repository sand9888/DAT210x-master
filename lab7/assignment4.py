...
def drawPlots(model, X_train, X_test, y_train, y_test, wintitle='Figure 1'):
	...
	for col in range(num_columns):
		for row in range(num_columns):
			...
			model.fit(X_train_bag, y_train.ravel())
			round(model.score(X_test_bag, y_test) * 100, 3)
...
X = pd.read_csv('C:/DAT210x-master/Module6/Datasets/wheat.data')
X = X.drop('id', axis=1)
X = X.dropna(axis=0)
# print(X.head(10))
...
dec_tree = tree.DecisionTreeClassifier(max_depth=9, random_state=2)
dec_tree.fit(X_train,y_train.values.ravel())


benchmark(knn, X_train, X_test, y_train.ravel(), y_test.ravel(), 'KNeighbors')
drawPlots(knn, X_train, X_test, y_train.ravel(), y_test.ravel(), 'KNeighbors')
